...
function iniBct(theWrapper){
    ...
    //click events for metadata section heading accordions
    /*$bctMetadata.find('h4').click(function(e){
...
function sqOpenMetadataPanel(assetId){
    ...
    //set the active metadata panel tag (if there is one)
    $('.bct-metadata.open').find('.bct-tabs .nav-link.active').each(function(){
        setActiveMetadataTab(this.id);
    });
}
